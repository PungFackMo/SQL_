-- Documents\dev\database\workspace\chapter04\chapter04-001.sql

-- sql고급
-- 01 내장함수
-- sql 내장 함수
-- sql 내장 함수는 다음과 같이 select 절과 where절, update절등에서
-- 모두 사용할 수 있다
-- select 함수명(인자1, 인자2, ...)
-- from 테이블 이름
...
-- 오라클에서 제공하는 내장 함수
-- 숫자 함수
-- sql문에서는 수학의 기본적인 사칙 연산자(+,-,*,/)와 나머지(%)연산자 기호를
-- 그대로 사용한다
-- 사직 연산자 외에도 여러 연산자가 있다
-- 이러한 연산자 중 사용 빈도가 높은 것을 내장 함수로 형태로 제공한다
-- abs함수 - 숫자의 절댓값 계산
-- -78과 +78의 절댓값을 구하시오.
...
-- nvl함수
-- nvl함수는 null 값을 다른 값으로 대치하여
-- 연산하거나 다른 값으로 출력
-- null값을 임의의 다른 값으로 변경할 수 있다
-- nvl(속성, 값)형식으로 사용
-- 속성이 null 값이면 값으로 대치한다
