...
create tablespace md_tbs
    datafile 'C:\Users\ITPS\Documents\dev\database\md_tbs_data01.dbf'
    size 10m;
...
-- 권한 관리 //0429
-- mdguest, mdguest2 사용자에게 데이터베이스 접속 및
-- 테이블 생성(resource) 권한, 
...
commit;
-- pdb1_madang 계정으로 접속
-- mdguest에게 book 테이블의 select 권한을 부여하시오
grant select on book to mdguest;

-- mdguest에게 customer 테이블의 select, update 권한을
-- with grant option과 함계 부여하시오.
grant select, update on customer to mdguest with grant option;

-- public은 모든 사용자에게 권한을 부여하는 명령이다
-- orders테이블을 모든 사용자가 select 할 수 있도록 권한을 부여하시오,.
grant select on orders to public;

-- 권한 취소 -revoke
-- grant 문으로 허가한 권한을 취소, 회수하는 명령이다.
-- 문법
/*
    revoke 권한 [(컬럼[,...n])[,...n]
